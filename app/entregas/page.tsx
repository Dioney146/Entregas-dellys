...
export default async function DashboardPage() {
  let indicadores = { total: 0, entregues: 0, ocorrencias: 0, naoEntregues: 0, fluvial: 0, rodoviario: 0 };
          ...
          <tbody>
            {entregas.slice(0, 25).map((e) => (
              <tr key={e.id} className="border-t border-slate-700">
                ...
                <td className="p-3">
                  <span className={`px-2 py-1 rounded-full text-xs text-white ${STATUS_LABEL[e.status]?.cor ?? "bg-slate-600"}`}>
